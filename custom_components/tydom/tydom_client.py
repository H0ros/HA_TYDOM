...
def _build_ssl_context() -> ssl.SSLContext:
    ...
    # sur OpenSSL 3.x (Python 3.12+) qui le désactive par défaut
    ctx.options |= getattr(ssl, "OP_LEGACY_SERVER_CONNECT", 0x4)
    ctx.options |= 0x4
    # Abaisser le niveau de sécurité pour autoriser les vieilles suites de chiffrement
    try:
...
class TydomClient:
    ...
    async def connect(self) -> bool:
        ...
        # Créer les streams asyncio sur le SSLSocket existant
        # On ne peut pas passer un SSLSocket directement à open_connection —
        # il faut créer le transport/protocol manuellement
        try:
            loop = asyncio.get_event_loop()
